  ...
  try {
    ...
    if (snapshot.empty) {
      ...
      fiches.forEach(function(f) {
        ...
        if (f.googleDriveLink && !f.googleDriveLinkNom) {
          f.googleDriveLinkNom = 'Document PDF';
          fichesRef.doc(f.id).set(f);
        }
...
function loadFiches() {
  ...
  fiches.forEach(function(f) {
    ...
    // Migrate googleDriveLinkNom for existing links
    if (f.googleDriveLink && !f.googleDriveLinkNom) {
      f.googleDriveLinkNom = 'Document PDF';
    }
    // Migration contenu fiche pension (id:2) vers version belge
...
function saveFiches() {
  try {
    ...
    console.error('saveFiches FAILED:', e);
  }
}
...
function openStandardModal(fiche) {
  ...
  var standardLinkHTML = renderPdfLink(fiche.googleDriveLink, fiche.googleDriveLinkNom);
...
function exportFichePdf(fiche) {
  ...
  if (fiche.googleDriveLink) {
    var nomPdf = fiche.googleDriveLinkNom || 'Document PDF';
    pdfLinksHtml += '<a href="' + escHtml(fiche.googleDriveLink) + '" target="_blank" style="display:inline-block;margin:4px 8px 4px 0;padding:6px 14px;background:#DC2626;color:#fff;text-decoration:none;border-radius:4px;font-size:11px;">' + escHtml(nomPdf) + '</a>';
  }
  var liensHtml = '';
...
function openSuiviModal(fiche, activeTab) {
  ...
  for (var i = 0; i < tabs.length; i++) {
    ...
    tabsHtml += '<button class="suivi-tab' + activeClass + '" onclick="switchSuiviTab(this,\'' + t + '\')">' + tabLabels[i] + '</button>';
...
function suiviInfoContent(fiche) {
  ...
  var cpptCount = (fiche.cppt && fiche.cppt.reunions) ? fiche.cppt.reunions.length : 0;
  var qualiteCount = (fiche.qualiteBatiments && fiche.qualiteBatiments.visites) ? fiche.qualiteBatiments.visites.length : 0;
  var spfCount = (fiche.spfBienEtre && fiche.spfBienEtre.inspections) ? fiche.spfBienEtre.inspections.length : 0;

  // Resume
  var resumeHTML = (fiche.resume && fiche.resume.trim()) ? '\
    <div class="suivi-section"><h4 class="suivi-section-title">📝 Résumé</h4><div class="suivi-resume-content">' + fiche.resume + '</div></div>' : '';

  // Pdfs annexes
  var pdfs = fiche.pdfs || [];
  var docsHTML = '';
  if (pdfs.length > 0 || fiche.googleDriveLink) {
    docsHTML = '<div class="suivi-section"><h4 class="suivi-section-title">📎 Documents</h4>';
    if (fiche.googleDriveLink) {
      docsHTML += renderPdfLink(fiche.googleDriveLink, fiche.googleDriveLinkNom);
    }
    if (pdfs.length > 0) {
  ...
  return '\
    <div class="modal-body" style="padding:0;">\
      <div class="suivi-info-fields">\
        ...
        <button class="save-btn" onclick="saveSuiviInfo(\'' + id + '\')" style="align-self:flex-end;padding:6px 16px;font-size:0.8rem;">Enregistrer</button>\
      </div>\
      <div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:14px;margin-top:10px;">\
        <div style="flex:1;min-width:120px;background:#F0F9FF;border-radius:8px;padding:10px;text-align:center;">\
          <div style="font-size:1.2rem;font-weight:800;color:var(--slfp-blue);">' + cpptCount + '</div>\
          <div style="font-size:0.7rem;color:#6B7280;">Réunions CPPT</div>\
        </div>\
        <div style="flex:1;min-width:120px;background:#F0FDF4;border-radius:8px;padding:10px;text-align:center;">\
          <div style="font-size:1.2rem;font-weight:800;color:#059669;">' + qualiteCount + '</div>\
          <div style="font-size:0.7rem;color:#6B7280;">Visites Qualité</div>\
        </div>\
        <div style="flex:1;min-width:120px;background:#FFF7ED;border-radius:8px;padding:10px;text-align:center;">\
          <div style="font-size:1.2rem;font-weight:800;color:#EA580C;">' + spfCount + '</div>\
          <div style="font-size:0.7rem;color:#6B7280;">Inspections SPF</div>\
        </div>\
      </div>\
      ' + resumeHTML + '\
      ' + infosHTML + '\
...
function suiviCpptContent(fiche) {
  ...
  return `
    <div style="display:flex;align-items:flex-end;margin-bottom:12px;gap:8px;flex-wrap:wrap;">
      ...
      <button class="btn-export" onclick="exportSectionPdf('${fiche.id}','cppt')" style="margin-left:auto;">Export PDF</button>
    </div>
    <table>
      <thead><tr><th>Date</th><th>Ordre du jour</th><th>Décisions</th><th>Actions</th></tr></thead>
      <tbody>${rows}</tbody>
    </table>
  `;
}
...
function uploadCpptPdf(ficheId, index) {
  ...
  input.onchange = function(e) {
    ...
    reader.onload = function(ev) {
      ...
      if (!fiche) return;
      getSuiviData(fiche);
      const r = fiche.cppt.reunions[index];
      if (!r) return;
      if (!r.pdfs) r.pdfs = [];
      r.pdfs.push({ name: file.name, data: ev.target.result });
      fiche.dateMiseAJour = new Date().toISOString().split('T')[0];
      saveFiches();
...
function suiviQualiteContent(fiche) {
  ...
  return `
    <div style="display:flex;align-items:flex-end;margin-bottom:12px;gap:8px;flex-wrap:wrap;">
      ...
      <button class="btn-export" onclick="exportSectionPdf('${fiche.id}','qualite')" style="margin-left:auto;">Export PDF</button>
    </div>
    ${html}
  `;
}
...
function uploadQualitePdf(ficheId, index) {
  ...
  input.onchange = function(e) {
    ...
    reader.onload = function(ev) {
      ...
      if (!fiche) return;
      getSuiviData(fiche);
      const v = fiche.qualiteBatiments.visites[index];
      if (!v) return;
      if (!v.pdfs) v.pdfs = [];
      v.pdfs.push({ name: file.name, data: ev.target.result });
      fiche.dateMiseAJour = new Date().toISOString().split('T')[0];
      saveFiches();
      showNotification('PDF ajouté');
      openSuiviModal(fiche, 'qualite');
...
function suiviSpfContent(fiche) {
  ...
  return `
    <div style="display:flex;align-items:flex-end;margin-bottom:12px;gap:8px;flex-wrap:wrap;">
      ...
      <button class="btn-export" onclick="exportSectionPdf('${fiche.id}','spf')" style="margin-left:auto;">Export PDF</button>
    </div>
    ${html}
  `;
}
...
function uploadSpfPdf(ficheId, index) {
  ...
  input.onchange = function(e) {
    ...
    reader.onload = function(ev) {
      ...
      if (!fiche) return;
      getSuiviData(fiche);
      const insp = fiche.spfBienEtre.inspections[index];
      if (!insp) return;
      if (!insp.pdfs) insp.pdfs = [];
      insp.pdfs.push({ name: file.name, data: ev.target.result });
      fiche.dateMiseAJour = new Date().toISOString().split('T')[0];
      saveFiches();
      showNotification('PDF ajouté');
      openSuiviModal(fiche, 'spf');
...
function openAddModal() {
  ...
  document.getElementById('ficheGoogleDriveLink').value = '';
  document.getElementById('ficheGoogleDriveLinkNom').value = '';
  setResumeContent('');
  setPdfEntries([]);
  setLiensEntries([]);
...
function editFiche(id) {
  ...
  document.getElementById('ficheGoogleDriveLink').value = fiche.googleDriveLink || '';
  document.getElementById('ficheGoogleDriveLinkNom').value = fiche.googleDriveLinkNom || '';
...
function handleFormSubmit(e) {
  ...
  var ficheData = {
    ...
    fichierPdf: null,
    pdfNom: null,
    googleDriveLink: document.getElementById('ficheGoogleDriveLink').value || null,
    googleDriveLinkNom: document.getElementById('ficheGoogleDriveLinkNom').value || null,
...
async function saveAndClose(ficheData, id) {
  ...
  if (useFirebase) {
    var success = await saveFicheToFirebase(ficheData);
    if (!success) return;
  }
